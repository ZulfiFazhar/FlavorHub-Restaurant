...
function PembayaranCard({ curPelanggan, setCurPelanggan, supabase }) {
  ...
  const handleClickBayar = async () => {
    // Konfirmasi bayar
    const yakinBayar = confirm(
      `Konfirmasi bayar untuk pelanggan atas nama : ${curPelanggan.nama_pemesan}`
    );
    ...
    const randomUang =
      Math.floor(Math.random() * ((70000 - 5000) / 1000 + 1)) * 1000 + 5000;
  ...
  return (
    <div className="border border-black rounded-md px-4 py-2 mt-3 flex flex-col">
      <div className="flex justify-between items-start">
        <div>
          <div>Nama : {curPelanggan.nama_pemesan}</div>

          <div>No meja : {curPelanggan.nomor_meja}</div>
        </div>
        ...
        <div className="flex items-center">
          ...
          <div className="flex">
            <div className="flex items-center mr-3">
              ...
              />
              <label htmlFor="qris" className="hover:cursor-pointer">
                qris
              </label>
            </div>
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="cash" className="hover:cursor-pointer">
                cash
              </label>
            </div>
          </div>
      ...
      <div className="border-b border-black rounded-md mt-3">
        <div className="flex justify-between">
          ...
          <div className="mr-1">Harga (Rp)</div>
        </div>
        {curPelanggan.pesanan.map((psn) => {
          return (
            <div
              key={Math.random()}
              className="border-t border-x border-black px-2 rounded-md flex justify-between"
            >
              <div>
      ...
      <div className="self-end mr-1">
        Total tagihan : {curPelanggan.total_harga}
      </div>

      <button
        className="border border-black px-2 rounded-md self-end green-custom mt-7 hover:bg-green-600"
