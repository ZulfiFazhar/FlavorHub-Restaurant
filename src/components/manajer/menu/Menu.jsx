    ...
    <div className="min-h-screen flex">
      <div className="w-2/3 p-10">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-stretch ml-6">
            ...
            <button
              className={`ml-1 rounded-xl bg-red-400 text-white px-2 hover:bg-red-600 pb-[0.1rem] ${
                cariMenu == "" && "bg-white hover:bg-white cursor-default"
              }`}
          ...
          <button
            className="text-center py-1 px-2 w-40 bg-orange-400 hover:bg-orange-500  transition ease-out rounded-md text-white"
            onClick={() => setBukaDetam((bd) => "tambah")}
          >
            + Tambah Menu
          </button>
        </div>
        ...
        <div className="mt-5 overflow-y-auto max-h-[38rem] flex flex-wrap justify-between *:mb-5 bg-slate-100 p-5 rounded-md h-5/6">
          {filteredMenu?.map((mn) => {
            return (
              <div
                key={mn.id}
                className="flex flex-col bg-slate-200 rounded-md p-2 w-32 h-72 mr-2"
              >
                <div className="relative w-full min-h-20 h-20">
                  <Image
                    src={`/menu/${
                      mn.foto
                        ? mn.foto
                        : "placeholder/menu-foto-placeholder.jpg"
                    }`}
                    alt="Foto menu"
                ...
                <p className="text-[0.6rem] mb-1 h-full overflow-y-hidden">
                  {mn.deskripsi} daging sapi denak sekali dengan tambahan warna
                  yang enak
                </p>

                <div className="flex justify-between mb-2">
                  <p className="text-xs">
                    <span className="text-orange-custom">Rp. </span> {mn.harga}
                  </p>
                </div>

                <button
                  className="mb-2 px-2 py-1 text-xs text-white rounded-md bg-green-600 hover:bg-green-800 transition-all ease-out"
                  onClick={() => handleClickMenuCard(mn)}
                >
                  detail
                </button>

                <button
                  className="px-2 py-1 text-xs text-white rounded-md bg-orange-400 hover:bg-orange-600 transition-all ease-out"
                  onClick={() => handleClickEdit(mn)}
                >
                  edit
                </button>
              </div>
      ...
      {bukaDetam == "none" && <div className="w-1/3"></div>}
      {bukaDetam == "tambah" && (
        <TambahMenu
          key={Math.random()}
          supabase={supabase}
